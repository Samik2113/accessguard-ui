...
module.exports = async function (context, req) {
  try {
    ...
    if (search) { query += " AND CONTAINS(UPPER(c.entitlement), @s)"; parameters.push({ name: "@s", value: search }); }

    const c = new CosmosClient(conn).database("appdb").container("entitlements"); // PK: /appId
    ...
    context.log.error("entitlements-get error:", err?.stack || err);
